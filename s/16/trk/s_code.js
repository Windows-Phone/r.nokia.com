/* SiteCatalyst code version: H.24.2. 09022012
Copyright 1996-2011 Adobe, Inc. All Rights Reserved
More info available at http://www.omniture.com */

/* INITIAL CONFIG : edit following variables before use */

    // Define global account (dev/prod)
    var s_account="nokiaglobalfwg0prod";
    var s=s_gi(s_account);

	try{
    //Define site specific report suite
    s.prop6=nkT.rsId;
    
    //Define initial pagename
    s.pageName=nkT.pName;
	} catch(e){ nkT = new Object(); }
...
s.channel="nokia:nokia.com:fw"; /* static value on all NCOM sites */

/* Method (FW,MW,MC) */
s.prop11=s.eVar11="fw";

/* Set server dynamically */
s.server=document.location.hostname;

/* Set cookie domain periods dynamically per site domain */
if(s.server.match(/\./g)!=null) {
s.fpCookieDomainPeriods=s.server.match(/\./g).length;
}
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,sis,sisx,dmg"
s.linkInternalFilters="javascript:,"+s.server+",localhost,file:,fusepump.com";
...
//opt-out starts
if(document.cookie.indexOf('utag_optout=1')==-1){ 

/* Check for alist */
s.alist = getCookie('s_alist');
if(s.alist!="") {	
	eraseCookie('s_alist');
} else { s.alist = ""; }
...
function s_doPlugins(s) {
	...
	if (s.getQueryParam('cid') && s.getQueryParam('cid').split("-").length > 7)
	{
		s.campaign = s.getValOnce(s.getQueryParam('cid', 'cid_once', 0));
	}
    
	if (s.getQueryParam('uid'))
	{
		s.eVar6 = s.getValOnce(s.getQueryParam('uid', 'uid_once', 0));
	}
	        	
	//capture and formulate intc
	if(s.getQueryParam('intc')) {
	     
        var intc = s.getQueryParam('intc').split("-");
		    if(intc[0]="na") { 
			intc[0] = nkT.pName;
			intc[0] = "ncom_" + intc[0].replace(/:/g,'_').replace(/-/g,'_'); 			  
			} 
		    intc[4] = intc[4].replace(/-/g,'_').replace(/:/g,'_');
		    intc[6] = "ncom_" + intc[6].replace(/:/g,'_').replace(/-/g,'_');
		  
			var intcFull=intc[0]+"-"+intc[1]+"-"+intc[2]+"-"+intc[3]+"-"+intc[4]+"-"+intc[5]+"-"+intc[6]+"-"+intc[7]+"-"+intc[8]+"-"+intc[9];
		    s.prop26=s.eVar26=s.getValOnce(intcFull, 'intc_cookie', 0);
	        s.prop49=s.eVar49=intc[9];
		    if(s.prop49!="") { s.events=s.apl(s.events,'event17',',',2); }
    }
	
    if(nkT.sLang && nkT.sCountry) { s.prop23=nkT.sCountry + ":" + nkT.sLang; }
	...
	if (s.pageName && s.pageName!='')
	{
		...
		/* Ensure that s.pageName is all in lowercase */
		s.pageName = s.pageName.toLowerCase();
    
		/* Add channel to pagename if it does not have it already */
		if(!s.pageName.match(s.channel)) {
		s.pageName = s.channel + ":" + s.pageName;
		}

		s.events=s.apl(s.events,'event1',',',2);
		s.hier1=s.eVar7=s.pageName;
		        
		var splitPageName = s.pageName.split(":");
		var splitSegments = s.pageName.split(":").length;
		s.prop1=splitPageName[0];
		for (i=1;i<=splitSegments-2;i++) {
		s.prop1=s.prop1 + ":" + splitPageName[i];
		}
		s.eVar1=s.prop1;
	
		if (splitSegments > 5)
		{
			s.eVar2=s.prop2=splitPageName[0] + ":" + splitPageName[1] + ":" + splitPageName[2] + ":" + splitPageName[3] + ":" + splitPageName[4] + ":" + splitPageName[5];
		}
		if (splitSegments > 4)
		{
		s.eVar3=s.prop3=splitPageName[0] + ":" + splitPageName[1] + ":" + splitPageName[2] + ":" + splitPageName[3] + ":" + splitPageName[4];
		}
		s.eVar4=s.prop4=splitPageName[0] + ":" + splitPageName[1] + ":" + splitPageName[2] + ":" + splitPageName[3];
		
		//Read productId from meta tag on support pages
		if(s.pageName.match('support')) {
			var metaTags = document.getElementsByTagName('meta');
			var metaTagLength=metaTags.length;
			for(var i=0;i<metaTagLength;i++) { 
			if(metaTags[i].name == 'ncsearch:productid') { s.eVar27=metaTags[i].content; }
			}
		}
		
	}
	
	if(s.alist!="" && s.alist!=null) { s.prop40=s.eVar17=s.alist; }
	s.prop39=s.eVar39=s.getPreviousValue(s.prop40,'gpv_alist','event1');
	if(s.prop39=="no value" || s.prop39==s.prop40 || !s.prop49) { delete s.prop39; delete s.eVar39; }
	s.prop34=s.eVar34=s.cg;
	if(s.cg==2) { s.events=s.apl(s.events,'event18',',',2); }
	  
	s.detectRIA('s_ria','prop24','prop42','','','1');
	
	s.eVar22=s.getNewRepeat(30,'s_newrep');
	s.eVar22=s.getValOnce(s.eVar22,'s_newrep_once',0);
	if (s.eVar22){ 
    ...
    // Reset s.pageName if error page
    if(nkT.pageType=="errorPage") { delete s.pageName; s.pageType="errorPage"; }
	
	if(s.linkTrackVars.indexOf("prop23") == -1) { s.linkTrackVars = s.linkTrackVars + ",prop23,eVar23"; }
	if(s.linkTrackVars.indexOf("prop6") == -1) { s.linkTrackVars = s.linkTrackVars + ",prop6"; }
    
}
s.doPlugins=s_doPlugins

//opt out ends
} else { s.t = new Function(); s.tl = new Function(); }
...
function setCookie(name,value,days) {
	...
	document.cookie = name+"="+value+expires+"; path=/; domain=.nokia.com;";
}
...
function nkTL(linkName,object) {
    
    s.prop48=s.eVar48=s.pageName;    
    s.linkTrackVars="prop6,prop9,prop48,eVar48";
    s.tl(object,'o',linkName);
    s_objectID=s.pageName + ":" + linkName;
}
/* custom link naming */
function nkNL(linkName,object) {
     s_objectID=s.pageName + ":" + linkName;
}
...
if(self == top) {
	s.loadModule("Survey");
	if(s_account=="nokiaglobalfwg0dev" && s.prop6) s.Survey.suites="nokia" + s.prop6 + "dev";
	if(s_account=="nokiaglobalfwg0prod" && s.prop6) s.Survey.suites="nokia" + s.prop6 + "prod";
}
