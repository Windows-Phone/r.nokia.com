/* SiteCatalyst code version: H.22.1. 20052011.2
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,sis,sisx"
s.linkInternalFilters="javascript:,"+s.server;
...
function s_doPlugins(s) {
	...
	if (s.getQueryParam('extcid'))
	{
		s.campaign = s.getValOnce(s.getQueryParam('extcid', 'cid_cookie', 0));
	}
	...
	if (s.pageName && s.pageName!='')
	{
		...
		if(!s.pageName.match(s.prefix)) {
		s.pageName = s.prefix + ":" + s.pageName;
		}
